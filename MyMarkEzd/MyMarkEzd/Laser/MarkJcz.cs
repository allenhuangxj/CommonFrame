...
namespace Laser_JCZ
{
    public class MarkJcz : LoadDll
    {
        ...
        public static bool InitLaser()
        {
            ...
            LmcErrCode Ret = LMC1_INITIAL(strEzCadPath, 0);
        ...
        public static bool MoveRotateAllEnt(double dOffsetX, double doffsetY, double dCenterX, double dCenterY, double dR)
        {
            ...
            lock (ObjectLock)
            {
                ...
                int nCount = LMC1_GETENTCOUNT();
                for (int i = 0; i < nCount; i++)
                {
                    LMC1_GETENTITYNAME(i, array);
                    string text = new string(array);
                    text = text.Replace("\0", "");
                    string text2 = text + "moveallent" + i.ToString();
                    LMC1_SETENTITYNAME(i, text2);
                    RoTateEnt(text2, dCenterX, dCenterY, dR);
                    MoveEnt(text2, dOffsetX, doffsetY);
                }
            }
        ...
        public static bool RoTateEnt(string strEntName, double dAngle)
        {
            ...
            lock (ObjectLock)
            {
                GetEntPos(strEntName, ref dCenx, ref dCeny);
                LmcErrCode Ret = LMC1_ROTATEENT(strEntName, dCenx, dCeny, dAngle);
                if (LmcErrCode.LMC1_ERR_SUCCESS == Ret)
                    return true;
                else
        ...
        public static bool GetEntityNameByIndex(int nIndex, ref string strName)
        {
            ...
            lock (ObjectLock)
            {
                ...
                if (LmcErrCode.LMC1_ERR_SUCCESS == Ret)
                {
                    strTempName = new string(strTemp);
                    strName = strTempName.Trim("\0".ToCharArray());
                }
                else
                {
                    mLastError = Ret;
        ...
        public static bool GetPenParam2(int nPenNo,//要设置的笔号(0-255)					 
           ref int nMarkLoop,//加工次数
           ref double dMarkSpeed,//标刻次数mm/s
           ref double dPowerRatio,//功率百分比(0-100%)	
           ref double dCurrent,//电流A
           ...
           ref double bWobbleDist)//抖动间距
        {
            lock (ObjectLock)
            {
                LmcErrCode Ret = LMC1_GETPENPARAM2(nPenNo, ref nMarkLoop,//加工次数
                ref dMarkSpeed,//标刻次数mm/s
                ref dPowerRatio,//功率百分比(0-100%)	
                ref dCurrent,//电流A
                ref nFreq,//频率HZ
                ...
                ref nJumpPosTC, //跳转位置延时us
                ref nJumpDistTC,//跳转距离延时us	
                ref dPointTime, // 打点时间 ms
                ref nSpiWave, //spi波形选择
                ref bWobbleMode, //抖动模式
                ref bWobbleDiameter, //抖动直径
                ref bWobbleDist); //抖动间距
        ...
        public static bool MarkEntity(string strEntityName)
        {
            ...
            lock (ObjectLock)
            {
                LmcErrCode Ret = LMC1_MARKENTITY(strEntityName);
                if (LmcErrCode.LMC1_ERR_SUCCESS == Ret)
                    return true;
                else
                {
        ...
        public static bool CopyEnt(string strSourceName, string strDesName)
        {
            ...
            lock (ObjectLock)
            {
                LmcErrCode Ret = LMC1_COPYENT(strSourceName, strDesName);
                if (LmcErrCode.LMC1_ERR_SUCCESS == Ret)
                    return true;
                else
                {
